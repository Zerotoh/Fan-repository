...
def _parse_running_dynamics_from_details(details):
    """从 get_activity_details 的逐样本时间序列里还原跑姿动力学。

    Garmin 对手表原生（腕部）跑步动力学，summaryDTO 里 often 不聚合 avgStrideLength 等字段，
    但它们以 direct* 指标存在于 activityDetailMetrics 时间序列（stride / vertical oscillation /
    vertical ratio / ground contact time）。这里按 metricDescriptors 的 key 取逐样本均值还原。

    返回字段（与旧 summaryDTO 路径保持一致）:
      stride_length_m / vertical_oscillation_cm / vertical_ratio / ground_contact_ms / cadence_spm
    """
    if not isinstance(details, dict):
        return None
    adm = details.get("activityDetailMetrics") or []
    mds = details.get("metricDescriptors") or []
    if not adm or not mds:
        return None
    idx = {}
    ...
    def series_mean(key, skip_zero=True):
        i = idx.get(key)
        if i is None:
            return None
        vals = []
        for rec in adm:
            ...
            if not isinstance(m, list) or len(m) <= i:
                continue
            v = m[i]
            if v is None:
                continue
    ...
    stride = series_mean("directStrideLength")       # cm
    vo = series_mean("directVerticalOscillation")     # cm
    vr = series_mean("directVerticalRatio")           # %
    gct = series_mean("directGroundContactTime")      # ms
    cad = series_mean("directDoubleCadence")          # spm（双足步频）
    out = {}
    if stride is not None:
        out["stride_length_m"] = round(stride / 100.0, 2)
    if vo is not None:
        out["vertical_oscillation_cm"] = round(vo, 2)
    if vr is not None:
        out["vertical_ratio"] = round(vr, 2)
    if gct is not None:
        out["ground_contact_ms"] = round(gct, 1)
    if cad is not None:
        out["cadence_spm"] = round(cad, 1)
    return out or None
...
def cmd_detail(args):
    ...
    try:
        ...
        # Running dynamics (跑姿 / 跑步动力学)
        # 需要 HRM-Run/Pro 心率带或支持腕部动力学的手表（如 FR255/265/965、Fenix7/Epix）。
        dyn = {}
        ...
        # 若 summaryDTO 未聚合跑姿字段（常见：手表原生腕部动力学只落在逐样本时间序列），
        # 则从 get_activity_details 的 activityDetailMetrics 取均值补全。
        if not dyn:
